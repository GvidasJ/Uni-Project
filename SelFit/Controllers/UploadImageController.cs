...
public class UploadImageController : ControllerBase
{
    ...
    [HttpPost("upload-picture"), Authorize(Roles = "Seller, Customer, Admin")]
    public async Task<ActionResult> UploadImage([FromForm] UploadImageDto uploadImageDto)
    {
        ...
        {
            return BadRequest("An image already exists for this product.");
        }
    ...
    [HttpPut("update-picture"), Authorize(Roles = "Seller, Customer, Admin")]
    public async Task<ActionResult> UpdatePicture([FromForm] UploadImageDto uploadImageDto)
    {
        ...
        try
        {
            ...
            {
                return BadRequest("Failed to change the existing image");
            }
    ...
    [HttpGet("get-picture/{productId}"), Authorize(Roles = "Seller, Admin")]
    ...
    [HttpDelete("delete-picture/{productId}"), Authorize(Roles = "Seller, Admin")]
    public async Task<ActionResult> DeletePicture([Required] Guid productId)
    {
        ...
        try
        {
            bool deleteSuccess = await _blobStorageService.DeleteFileAsync(product.ImageUrl);
            if (deleteSuccess == false)
            {
                return BadRequest("Failed to delete the existing image.");
            }
