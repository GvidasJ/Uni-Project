...
public class OrderController : ControllerBase
{
    ...
    [HttpPost("place-order"), Authorize(Roles = "Customer, Admin")]
    public async Task<ActionResult> PlaceOrder([FromBody] OrderDto orderDto)
    {
        ...
        if (seller == null)
        {
            return BadRequest($"Seller with ID {orderDto.SellerId} not found.");
        }
        ...
        if (newOrder == null)
        {
            return BadRequest("Order creation failed, check the order details.");
        }

        return Ok(newOrder);
    }

    [HttpPost("change-order-status/{orderId}"), Authorize(Roles = "Seller, Admin")]
    public async Task<ActionResult> ChangeOrderStatus([FromRoute] Guid orderId, [FromQuery] OrderStatusEnum orderStatus)
    {
        ...
        if (status == false)
        {
            return BadRequest("Failed to change order status");
        }

        return Ok("Order Status was changed successfully");
    }

    [HttpGet("orders-by-seller/{sellerId}"), Authorize(Roles = "Seller, Admin")]
    public async Task<ActionResult<List<ResponseOrder>>> GetOrdersBySellerId([FromRoute] Guid sellerId)
    {
        ...
        {
            return BadRequest("No orders found for the specified seller.");
        }

        return Ok(orders);
    }

    [HttpGet("get-order/{orderId}"), Authorize]
    ...
    [HttpDelete("delete-order/{id}"), Authorize(Roles = "Admin")]
